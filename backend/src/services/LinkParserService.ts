...
class LinkParserService {
  private patterns: Record<Platform, RegExp>
  ...
  private async getBilibiliMeta(url: string, bvid: string): Promise<VideoMeta> {
    try {
      const response = await fetch(`https://api.bilibili.com/x/web-interface/view?bvid=${bvid}`)
      const data = await response.json() as { code: number; data?: { title: string; pic: string; duration: number; owner: { name: string }; aid: number } }
  ...
  private async getYouTubeMeta(url: string, videoId: string): Promise<VideoMeta> {
    try {
      ...
      try {
        ...
        const invidiousData = await invidiousResponse.json() as { duration: number }
  ...
  private async getDouyinMeta(url: string, videoId: string): Promise<VideoMeta> {
  ...
  private async getWechatVideoMeta(url: string, videoId: string): Promise<VideoMeta> {
