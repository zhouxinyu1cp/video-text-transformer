...
import type { Transcript, TranscriptSegment, SpeakerMap, VideoMeta } from '@video-transcriber/shared'

export async function transcribeRoutes(app: FastifyInstance) {
  app.post('/api/transcribe', async (request, reply) => {
    const { sessionId, videoUrl, videoMeta } = request.body as {
      sessionId: string
      videoUrl: string
      videoMeta: VideoMeta
    }
    ...
    try {
      ...
      const finalVideoMeta: VideoMeta = {
